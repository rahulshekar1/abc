...
function sortByDistance() {
  ...
  const filteredFeatures = geojsonData.features.filter((feature) => {
    ...
    return (
      coords[0] >= bounds._sw.lng &&
      coords[0] <= bounds._ne.lng &&
      coords[1] >= bounds._sw.lat &&
      coords[1] <= bounds._ne.lat
    );
  });
  ...
  // Update the UI by rebuilding the location list with filtered data
  const listings = document.getElementById('listings');
...
map.on('load', () => {
  ...
  function makeGeoJSON(csvData) {
      ...
      (err, data) => {
        ...
        map.addLayer({
          ...
          paint: {
            ...
            'circle-color': '#CCCFFF', // color of circles
    ...
    let activePopup = new mapboxgl.Popup();
    map.on('mouseenter', 'locationData', (e) => {
      ...
      if (features.length > 0) {
        ...
        const popupContent = '<h3>' + feature.properties[config.popupInfo] + '</h3>';
